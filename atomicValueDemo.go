...
func main() {
	...
	box2.Store(v2) //这里并不会应发panic
	fmt.Printf("The value load from box is %v.\n", box.Load())
	fmt.Printf("The value load from box2 is %q.\n", box2.Load())
	fmt.Println()

	fmt.Printf("Copy box to box3.")
	box3 := box //原子值在真正使用后不应该被复制！
	fmt.Printf("The value from box3 is %v.\n", box3.Load())

	v3 := 123
	fmt.Printf("Stor %d to box2.\n", v3)
	...
	if ok {
		...
		//	box4.Store(v42) //这里会引发一个panic, 报告存储的值的类型不一致
	}
	fmt.Println()
	...
	fmt.Println("Store a value of the same type of box5")
	err = box5.Store(v41)
	if err != nil {
		fmt.Printf("error:%s\n", err)
	}
